...
You can use the [IOTA CLI client](../../developer/cli/client.mdx) to request tokens for your [address](get-address.mdx) by using the `iota client faucet` command. By default, the command will use the active network and address, but you can specify any values for these using the following parameters:
