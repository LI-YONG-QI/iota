...
You can use the  [IOTA Client CLI](../../developer/cli/client.mdx) to generate any number of addresses by using
the `iota client new-address <KeyScheme>`, where `<KeyScheme>` is one of the following:
...
the [IOTA Keytool CLI](../../developer/cli/keytool.mdx) by running the following command:
