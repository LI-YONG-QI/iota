...
The following steps demonstrate how to create a multisig transaction and then submit it against a local network using the [IOTA CLI](/developer/cli.mdx). A transaction can be the transfer of an object, the publish or upgrade of a package, the payment of IOTA, and so on. To learn how to set up a local network, see [Connect to a Local Network](/developer/getting-started/local-network.mdx).
