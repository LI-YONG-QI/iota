...
Functions that return a reference, such as [`std::option::borrow`](../../references/framework/move-stdlib/option.mdx#function-borrow), cannot be used in a PTB call but only in a smart contract.
...
Mint coins with a [`0x2::coin::TreasuryCap`](../../references/framework/iota-framework/coin.mdx#resource-treasurycap) and transfer them to the sender. Based on the [simple_token.move](https://github.com/iotaledger/iota/blob/develop/examples/move/token/sources/simple_token.move) example.
