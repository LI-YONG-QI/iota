...
fn extract_macros(
    ...
    // Prefer local module definitions to previous ones. This is ostensibly an
    // error, but naming should have already produced that error. To avoid
    // unnecessary error handling, we simply prefer the non-precompiled
    // definitions.
...
fn exp(context: &mut Context, ne: Box<N::Exp>) -> Box<T::Exp> {
    ...
    let (ty, e_) = match ne_ {
        ...
        NE::ExpDotted(usage, edotted) => {
            ...
            // If the type location would point to this expression (i.e., not to a struct
            // field or similar), we watnt to respan it to blame the overall
            // term for a type mismatch (because it likely has a `&`, `*`, or
            // `&mut `). If the type is from elsewhere, however, we prefer that
            // location.
...
fn resolve_index_funs_and_type(
    ...
    // NOTE: We don't do a visibility check here because we _just_ care about
    // computing the return type. The visibility check will happen later in
    // `exp_to_borrow_`.
...
fn process_exp_dotted(
    ...
    // `process_exp_dotted` in order to share definitions while enforcing when and
    // if autocompletion should occur.
    ...
    /// Looks up an index access and builds the appropriate form for later
    /// resolution. Always returns an `ExpDottedAccess::Index`.
    #[growing_stack]
...
fn exp_dotted_to_owned(
    ...
    // if matches!(&borrow_exp.exp.value, TE::AutocompleteAccess(..)) { return
    // borrow_exp; }
...
fn syntax_call_return_ty(
    ...
    // The subtyping takes care of this for us.

    // For the first argument, since it may be incorrectly mut/imm, we don't report
    // an error.
    ...
    // For the other arguments, failure should be reported. If it is, we also mark
    // the call as invalid, indicating a return type error.
    ...
    // chain: if a field or syntax index function is invalid or failed to
    // resolve, we hand up an error and propagate it, instead of re-attempting
    // to handle it at each step in the accessors (which would cause the user to
    // get a new error for each part of the access path). Similarly, if our call
    // arguments are wrong, the path is already bad so we're done trying to resolve
    // it and drop into this error case.
    if !valid {
...
fn expected_by_name_arg_type(
    ...
    // We need to return the subtyped type to properly remove the `Anything` in the
    // cases where it should be a specific type, e.g. |_| -> _ <: |'a| -> 'b
    // should return |'a| -> 'b In the case of an error, we give back tfun so
    // macro expansion continues to know that this argument is a lambda
