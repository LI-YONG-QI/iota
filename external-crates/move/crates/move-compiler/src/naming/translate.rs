...
// -------------------------------------------------------------------------------------------------
// These type definitions hold the information about module members, which we
// can retain and reuse. These are used to build up the actual resolution types
// returned during name resolution.
...
// -------------------------------------------------------------------------------------------------
// These type definitions are the result of a resolution call, based on the type
// of thing you are trying to resolve.
...
impl ResolvedType {
    /// Set the information for the module identifier and name to the ones
    /// provided. This allows name resolution to preserve location
    /// information and address names from the original name
...
impl ResolvedVariant {
    /// Set the information for the module identifier, enum_name, and name to
    /// the ones provided. This allows name resolution to preserve location
    /// information and address names from the original name onto the
    /// resolved one.
...
pub fn build_member_map(
    ...
    // NB: This checks if the element is present, and doesn't replace it if so. This
    // is congruent with how top-level definitions are handled for alias
    // resolution, where a new definition will not overwrite the previous one.
...
impl<'env> Context<'env> {
    ...
    /// Main module access resolver. Everything for modules should go through
    /// this when possible, as it automatically preserves location
    ...
    // Like resolve_local, but with an ICE on failure because these are precomputed
    // and should always exist. This also does not mark usage, as this is
    // ostensibly the binding form. This is so that we can walk through
    // or-patterns and reuse the bindings for both sides.
...
fn module(
    ...
    // simply for compilation to continue in the presence of errors, we remove the
    // duplicates
...
fn match_arm(context: &mut Context, sp!(aloc, arm): E::MatchArm) -> N::MatchArm {
    ...
    context.new_local_scope();
    // NB: we just checked the binders for duplicates and listed them all, so now we
    // just need to set up the map and recur down everything.
    ...
    // Guards are a little tricky: we need them to have similar binders, but they
    // must be different because they may be typed differently than the actual
    // binders (as they are always immutable references). So we push a new scope
    // with new binders paired with the pattern ones, process the guard, and
    // then update the usage of the old binders to account for guard usage.
    context.new_local_scope();
    ...
    // Next we compute the used guard variables, and add the pattern/guard pairs to
    // the guard binders. We assume we don't need to mark unused guard bindings
    // as used (to avoid incorrect unused errors) because we will never check
    // their usage in the unused-checking pass.
    //
    // We also need to mark usage for the pattern variables we do use, but we
    // postpone that until after we handle the right-hand side.
    ...
    // Then we visit the right-hand side to mark binder usage there, then compute
    // all the pattern binders used in the right-hand side. Since we didn't mark
    // pattern variables as used by the guard yet, this allows us to record
    // exactly those pattern variables used in the right-hand side so that we
    // can avoid binding them later.
        ...
        EP::ModuleAccessName(name, etys_opt) => {
            match context.resolve_pattern_term(name) {
                ...
                ResolvedPatternTerm::Constructor(ctor) => {
                    ...
                    match *ctor {
                        ResolvedConstructor::Struct(stype) => {
                            // No need to check is_empty / is_positional because typing will report
                            // the errors.
