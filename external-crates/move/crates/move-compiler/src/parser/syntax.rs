...
impl<'env, 'lexer, 'input> Context<'env, 'lexer, 'input> {
    ...
    /// Advances tokens until reaching an element of the stop set, recording
    /// diagnostics along the way (including the first optional one passed
    /// as an argument).
    ...
    F: Fn(&mut Context) -> Result<R, Box<Diagnostic>>,
{
    ...
    if let Err(diag) = consume_token(context.tokens, start_token) {
        ...
        // advance token past the starting one but something went wrong, still there is
        // a chance parse the rest of the list
        ...
        if context.tokens.at(end_token) {
            // at the end of the list - consume end token and keep parsing at the outer
            // level
            context.advance();
            return vec![];
        }
    ...
    F: Fn(&mut Context) -> Result<R, Box<Diagnostic>>,
{
    ...
    while !context.tokens.at(end_token) {
        ...
        // The stop set check is done at the end of the loop on purpose as we need to
        // attempt parsing before checking it. If we do not, in the best case,
        // we will get a less meaningful error message if the item belongs to
        // the token set incorrectly (e.g., `fun` keyword), and in the worst
        // case, we will get an error in the correct code (e.g., if a function argument
        // is named `entry`)
        if context.at_stop_set() {
            break;
        }
...
/// Attempts to skip tokens until the end of the item in a series of separated
/// (which started with an already consumed starting token) - looks for a
/// matched ending token or a token appearing after the separator. This helper
/// function is used when parsing lists and sequences.
fn advance_separated_items_error(
    ...
    loop {
        // adjusting tokens (replacing `<<` with `<`) makes sense only when parsing
        // lists and it would feel odd to also do this when using this helper
        // function to parse other things (e.g., sequences)
        if for_list {
            adjust_token(context.tokens, end_token);
        }
        if context.at_stop_set() {
...
//
// Expects the current token to be Tok::Identifier or Tok::RestrictedIdentifier
// and returns `Syntax::UnexpectedToken` for the current token if it is not.
...
//
//  (n in {0,1,2,3})
//  Returns a name and an indicator if we parsed a macro. Note that if `n > 3`,
// this parses those  and reports errors.
...
fn parse_name_access_chain_<'a, F: Fn() -> &'a str>(
    ...
    while context.tokens.peek() == Tok::ColonColon {
        ...
        let name = match parse_identifier(context) {
            Ok(ident) => ident,
            Err(_) => {
                // diagnostic for this is reported in path expansion (as a parsing error) when
                // we detect incomplete chain (adding "default" diag here would
                // make error reporting somewhat redundant in this case)
...
fn parse_attributes(context: &mut Context) -> Result<Vec<Attributes>, Box<Diagnostic>> {
    ...
    // Attaches the doc comments to the start of the location of the member that
    // these attributes/doc comments are attached to.
...
// Parse an optional ellipis token. Consumes and returns the location of the
// token if present, and returns `Ok(None)` if not present.
//      Ellipsis = "..."?
...
fn parse_sequence(context: &mut Context) -> Result<Sequence, Box<Diagnostic>> {
    ...
    while context.tokens.peek() != Tok::RBrace {
        // this helps when a sequence contains a comma-separated list without the ending
        // token (in which case the parser would be likely fast-forwarded to
        // EOF)
    ...
    // If we reached the stop set but did not find closing of the sequence (RBrace)
    // and we need to decide what to do. These are the two most likely possible
    // scenarios:
    //
    // module 0x42::M {
    //   fun t() {
    ...
    // }
    //
    // In the first case we encounter stop set's `use` as incorrect inner
    // definition, in the second case, we encounter `struct` as a legit
    // top-level definition after incomplete function above. We cannot magically
    // know which is which, though, at the point of reaching stop set, but still
    // need to make a decision on what to do, which at this point is to close the
    // current sequence and proceed with parsing top-level definition (assume
    // the second scenario).
...
// Parses a series of match arms, such as for a match block body "{"
// (<MatchArm>,)+ "}"
...
//    DatatypeTypeParameter = '<' Comma<TypeParameterWithPhantomDecl> ">" |
// <empty>
...
// Where the two "has" statements are mutually exclusive -- an enum cannot be
// declared with both infix and postfix ability declarations.
fn parse_enum_decl(
    attributes: Vec<Attributes>,
    start_loc: usize,
    modifiers: Modifiers,
...
//      VariantDecl = <Identifier> ("{" Comma<FieldAnnot> "}" | "("
// Comma<PosField> ")")
...
fn check_enum_visibility(visibility: Option<Visibility>, context: &mut Context) {
    let current_package = context.current_package;
    // NB this could be an if-let but we will eventually want the match for other
    // vis. support.
    match &visibility {
...
//          <Identifier> <OptionalTypeParameters>
// Where the two "has" statements are mutually exclusive -- a struct cannot be
// declared with both infix and postfix ability declarations.
fn parse_struct_decl(
    ...
    let mut abilities = if infix_ability_declaration_loc.is_some() {
        parse_infix_ability_declarations(context)
            .map_err(|diag| {
                // if parsing failed, assume no abilities present even if `has` keyword was
                // present
...
// Parse either just semicolon (for native structs) or fields and (optional)
// postfix abilities
...
fn parse_module(
    ...
    while context.tokens.peek() != Tok::RBrace {
        ...
        // If we see the `module` keyword, this is most-likely someone defining a second
        // module (erroneously), so we also bail in that case.
