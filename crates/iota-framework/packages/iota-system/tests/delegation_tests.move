...
fun test_add_remove_stake_flow() {
    ...
    scenario.next_tx(STAKER_ADDR_1);
    {
        ...
        system_state_mut_ref.request_add_stake(
            coin::mint_for_testing(60 * NANOS_PER_IOTA, ctx),
            VALIDATOR_ADDR_1,
            ctx,
        );

        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_1) == 100 * NANOS_PER_IOTA,
        );
        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_2) == 100 * NANOS_PER_IOTA,
        );
    ...
    scenario.next_tx(STAKER_ADDR_1);
    {
        ...
        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_1) == 160 * NANOS_PER_IOTA,
        );
        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_2) == 100 * NANOS_PER_IOTA,
        );
